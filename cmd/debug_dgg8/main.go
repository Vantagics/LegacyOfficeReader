...
func main() {
	...
	// Parse the first DgContainer (main document) at offset 501
	fmt.Println("=== DgContainer #1 (Main Document) ===")
	parseDgContainer(dggData, 501, 501+8+776)

	fmt.Println("\n=== DgContainer #2 (Header Document) ===")
	parseDgContainer(dggData, 1286, 1286+8+5251)
}

func parseDgContainer(data []byte, offset, end int) {
	// Skip the DgContainer header
	offset += 8
	...
	for offset+8 <= end {
		...
		if recType == 0xF004 { // SpContainer
			spid, pib, txid, hasCT := parseSpContainer(data, offset+8, childEnd)
			fmt.Printf("  SpContainer: spid=%d pib=%d txid=%d clientTextbox=%v\n", spid, pib, txid, hasCT)
		} else if recType == 0xF003 { // SpgrContainer
			fmt.Printf("  SpgrContainer len=%d\n", recLen)
			parseSpgrContainer(data, offset+8, childEnd)
		} else if recType == 0xF008 { // Dg
			if int(recLen) >= 8 {
			...
			fmt.Printf("  [0x%04X] %s ver=%d len=%d\n", recType, name, ver, recLen)
		}
...
func parseSpgrContainer(data []byte, offset, end int) {
	for offset+8 <= end {
		verInst := binary.LittleEndian.Uint16(data[offset:])
		recType := binary.LittleEndian.Uint16(data[offset+2:])
		recLen := binary.LittleEndian.Uint32(data[offset+4:])
		...
		if recType == 0xF004 { // SpContainer
			spid, pib, txid, hasCT := parseSpContainer(data, offset+8, childEnd)
			fmt.Printf("    SpContainer: spid=%d pib=%d txid=%d clientTextbox=%v\n", spid, pib, txid, hasCT)
		} else if recType == 0xF003 { // nested SpgrContainer
			fmt.Printf("    Nested SpgrContainer len=%d\n", recLen)
			parseSpgrContainer(data, offset+8, childEnd)
		} else {
			name := msoRecordName(recType)
			fmt.Printf("    [0x%04X] %s ver=%d len=%d\n", recType, name, ver, recLen)
...
func parseSpContainer(data []byte, offset, end int) (spid, pib, txid uint32, hasClientTextbox bool) {
	for offset+8 <= end {
		verInst := binary.LittleEndian.Uint16(data[offset:])
		recType := binary.LittleEndian.Uint16(data[offset+2:])
		recLen := binary.LittleEndian.Uint32(data[offset+4:])
